...
def CreateNFT(
    ...
    print("CreateNFT nft_id: ")
    print(nft_id)
    print("CreateNFT tx_id: ")
    print(tx_id)
...
def create_nft():
   nft_creator_address = request.args['dev_address']
   nft_creator_mnemonic = request.args['dev_mnemonic']
   nft_creator_pk = getPK(nft_creator_mnemonic)
   nft_reserve_address = request.args['nft_owner_address']
   nft_reserve_mnemonic = request.args['nft_owner_mnemonic']
   nft_reserve_pk = getPK(nft_reserve_mnemonic)
   token = request.args['token']
   print("token: " + token)
   address = request.args['ip_address']
   print("address: " + address)
   purestake_token = {'X-Api-key': token}
   print("purestake_token: " + 'X-Api-key : ' + token)
   client = algod.AlgodClient(token, address, headers=purestake_token)
   print(client)
   unit_name = request.args['unit_name']
   print("unit_name: " + unit_name)
   asset_name = request.args['asset_name']
   print("asset_name: " + asset_name)
   nft_url = request.args['nft_url']
   print("nft_url1: " + nft_url)
   nft_url = unquote_plus(nft_url)
   print("nft_url2: " + nft_url)
   nft_creator_pk=nft_creator_pk.replace(" ", "+")
   print("nft_creator_pk: " + nft_creator_pk)
   nft_reserve_pk=nft_reserve_pk.replace(" ", "+")
   print("nft_reserve_pk: " + nft_reserve_pk)
...
def make_sell_offer():
    ...
    nft_creator_pk = getPK(nft_creator_mnemonic)
    nft_reserve_address = request.args['nft_owner_address']
    nft_id = request.args['nft_id']
    token_id = request.args['token_id']
    sell_price = request.args['sell_price']
    token = request.args['token']
    address = request.args['ip_address']
    purestake_token = {'X-Api-key': token}
    client = algod.AlgodClient(token, address, headers=purestake_token)
...
def buy_nft_offer():
   nft_owner_address = request.args['nft_owner_address']
   # nft_owner_mnemonic = request.args['nft_owner_mnemonic']
   # nft_owner_pk = getPK(nft_owner_mnemonic)
   dev_address = request.args['dev_address']
   dev_mnemonic = request.args['dev_mnemonic']
   dev_pk = getPK(dev_mnemonic)
   buyer_address= request.args['buyer_address']
   buyer_mnemonic = request.args['buyer_mnemonic']
   buyer_pk = getPK(buyer_mnemonic)
   buy_price = request.args['buy_price']
...
def token_optin():
   receiver_pk = request.args['receiver_pk']
   token_id = request.args['token_id']
   token = request.args['token']
   address = request.args['ip_address']
   purestake_token = {'X-Api-key': token}
...
def nova_payment():
   ...
   receiver_address = request.args['receiver_address']
...
def test():
    ...
    dev_address = request.args['dev_address']
    print("테스트 된다!!" + dev_address)
    return jsonify(result='success', aa=dev_address)
